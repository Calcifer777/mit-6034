...
def alpha_beta_search(
    ...
    def loop(alpha, beta, board, depth):
        # Check end state
    ...
    # Iterate
    best_eval, best_move = loop(NEG_INFINITY, INFINITY, board, depth)
    if verbose:
        print "Alpha Beta: Decided on column %s with rating %s" % (str(best_move), str(best_eval))
    return best_move
...
def better_evaluate(board):
    ...
    # Check if end state
    if board.is_win() == me:  return INFINITY
    if board.is_win() == you: return NEG_INFINITY
    if board.is_game_over():  return 0
    # Else evaluate board
    score = 0
    # Adjust based on current state
    for c in board.chain_cells(me):  
        if len(c) > 1:
            score += len(c)**2
    for c in board.chain_cells(you):
        if len(c) > 1:
            score -= len(c)**2
    # Me longest chain is better than You longest chain
    score += (
        board.longest_chain(me) - board.longest_chain(you)
    )*1000
    # Prefer having your pieces in the center of the board.
    for row in range(6):
        for col in range(7):
