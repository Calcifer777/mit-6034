...
def minimax_find_board_value(board, depth, eval_fn,
    ...
    """
    Minimax helper function: Return the minimax value of 
    a particular board, given a particular depth to estimate to
    """
...
def minimax(board, depth, eval_fn = basic_evaluate,
    ...
    eval_fn -- (optional) the evaluation function to use to give a value to a leaf of the tree; 
                see "focused_evaluate" in the lab for an example

    Returns an integer, the column number of the column that the search determines you should add a token to
    """
    
    best_val = None  # List[rating, column]
